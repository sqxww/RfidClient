...
public class Main {
	public static void main(String[] args) throws Exception{
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					new RfidClient().connect(8080, "127.0.0.1");
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		CMessage loginReq = new CMessage();
		loginReq.setType((byte)1);
		loginReq.setBodyType((byte)1);
		loginReq.setSessionId(0L);
		loginReq.setMessageLength(32);
		loginReq.setUrlLength(0);
...
//
//				public void run() {
//					CMessage heartBeatMsg = new CMessage();
